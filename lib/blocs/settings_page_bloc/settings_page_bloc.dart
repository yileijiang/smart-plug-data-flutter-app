...
class SettingsPageBloc extends Bloc<SettingsPageEvent, SettingsPageState> {
  ...
  SettingsPageBloc()
      : super(WidgetsLoading()) {
    on<HomeAssistantAddressWidgetLoaded>(_mapHomeAssistantAddressWidgetLoadedEventToState);
    on<AccessTokenWidgetLoaded>(_mapAccessTokenWidgetLoadedEventToState);
    on<PushNotificationsSettingWidgetLoaded>(_mapPushNotificationsSettingWidgetLoadedEventToState);
    on<HomeAssistantConnectionWidgetLoaded>(_mapHomeAssistantConnectionWidgetLoadedEventToState);
  }

  void _mapHomeAssistantAddressWidgetLoadedEventToState(
      HomeAssistantAddressWidgetLoaded event, Emitter<SettingsPageState> emit) async {
  ...
  void _mapPushNotificationsSettingWidgetLoadedEventToState(
      PushNotificationsSettingWidgetLoaded event, Emitter<SettingsPageState> emit) async {
    pushNotificationsSettingLoaded = true;
    areAllWidgetsLoaded();
  }

  void _mapHomeAssistantConnectionWidgetLoadedEventToState(
      HomeAssistantConnectionWidgetLoaded event, Emitter<SettingsPageState> emit) async {
    homeAssistantConnectionLoaded = true;
    areAllWidgetsLoaded();
  }

  void areAllWidgetsLoaded() {
    if (homeAssistantAddressLoaded && accessTokenLoaded && pushNotificationsSettingLoaded && homeAssistantConnectionLoaded) {
